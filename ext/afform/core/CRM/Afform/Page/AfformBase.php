...
class CRM_Afform_Page_AfformBase extends CRM_Core_Page {
  ...
  public function run() {
    ...
    // The api will throw an exception if afform is not found (because of single)
    $afform = \Civi\Api4\Afform::get(FALSE)
      ->addWhere('name', '=', $pageArgs['afform'])
      ->addSelect('title', 'module_name', 'directive_name', 'navigation', 'server_route', 'is_public')
      ->execute()
      ->single();
