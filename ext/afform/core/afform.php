...
function afform_civicrm_post($op, $entityName, $id, $object, $params) {
  ...
  if ($entityName === 'CustomGroup' || $entityName === 'CustomField') {
    try {
      ...
      $backtrace = \CRM_Core_Error::formatBacktrace(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS));
      \Civi::log()->warning("Afform cache clear failed after {$op} of {$entityName} {$id}\n{$backtrace}", );
    }
  }
}
