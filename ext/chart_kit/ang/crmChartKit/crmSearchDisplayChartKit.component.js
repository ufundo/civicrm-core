    ...
    controller: function ($scope, $element, searchDisplayBaseTrait, chartKitChartTypes, chartKitColumnConfig) {
      ...
      this.buildCrossfilter = () => {
        const dataPoints = this.results.map((record, i) => {
          ...
          this.getColumns().forEach((col) => {
            ...
            dataPoint[col.index] = col.toDataValue(resultValue);
          });
      ...
      this.loadChartData = () => {
        ...
        if (this.chartType.loadChartData) {
          ...
          this.chart
            ...
            .valueAccessor(this.getFirstColumnForAxis('y').getDataValue);
        }
      };
      ...
      this.formatCoordinateGrid = () => {
        ...
        if (xCol.scaleType !== 'date') {
          this.chart.xAxis().tickFormat((v) => xCol.renderDataValue(v));
        }
        ...
        if (leftYCols.length === 1) {
          this.chart.yAxis().tickFormat((v) => leftYCols[0].renderDataValue(v));
        }
        ...
        if (supportsRightYAxis) {
          ...
          if (rightYCols.length === 1) {
            this.chart.rightYAxis().tickFormat((v) => rightYCols[0].renderDataValue(v));
          }
          if (rightYCols) {
